...
int main() {
    ...
    vector<unordered_map<int, int>> drank(M+1);
    
    FOR(i,0,D) {
        int p, m, t; cin >> p >> m >> t;
        if (!drank[m].count(p) || drank[m][p] > t) drank[m][p] = t;
    }
    ...
    FOR(m,1,M+1) {
        int cnt = 0, sick_cnt = 0;

        for (auto p : drank[m]) {
            if (sick[p.fi] == -1) cnt++;
            else if (sick[p.fi] > p.se) cnt++, sick_cnt++;
        }

        if (sick_cnt == S) ckmax(ans, cnt);
    }
